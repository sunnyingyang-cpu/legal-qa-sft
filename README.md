...
4. **鲁棒性实验两轮均无效**：扰动集生成正确（60 条），但推理时 Colab 磁盘上的数据文件为旧副本（日志 `Num examples = 300`，预测与测试集逐字相同）。修复：重跑 `eval/robustness.py` 与 `configs/infer_robustness.yaml` 两格，**验证日志出现 `Num examples = 60`** 再评指标；
5. **指标盲区**：引用类指标测不出选项/推理层面的对错（案例三），需 LLM-as-Judge 或选项准确率补齐；
6. **P95 端到端未达标**：根因与修复见 §7。

## 9. 下一步（按优先级）

1. **RAG 注入法条库**——直接攻击"相邻条文混淆"这一最大错误来源，预期对事实正确率的收益大于一切调参；
2. **重跑鲁棒性实验**（8 分钟，验证点见 §8.4）；
...
| ft-1.5b-robust | 扰动集 60 条 | — | — | — | — | — | — | 两轮无效（§8.4），待重跑 |
